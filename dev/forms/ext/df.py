...
# add entry functionality backend
...
# update entry functionality backend
class UpdateEntryForm(DynamicForm):
    def __init__(self,**kwargs):
        # отримуємо primary key  рядка, який оновлюється
        self.item_pk = [(c['name'],c) for c in kwargs['columns'] if c['pk'] == 1]
        DynamicForm.__init__(self,**kwargs)
    ...
    def q(self,db_engine):
        ...
        for cname in self.colnames:
            ...
            for item in val:
                prepared.append("'{0}'".format(item) if not isinstance(item,(int,)) else str(item))
        qdata=[]
        for elem in zip(col,prepared):
            qdata.append('{0}={1}'.format(elem[0],elem[1]))
        query = "update {0} set {1} where ".format(self._table,','.join(qdata),'='.join(self.item_pk[0][0],self.item_pk[0][1]))
        db_engine.q(query)
